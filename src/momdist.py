...
# def initialize(dim):
# 	# For now full rank, parameterize by chol factor M
# 	return np.eye(dim)

# def sample(rng_key, eta, prev, params):
# 	# Params is just a matrix
# 	M = params
# 	M = np.tril(M)
# 	dim = M.shape[0]
# 	rho_indep = np.dot(M, jax.random.normal(rng_key, (dim,)))
# 	if prev is None:
# 		rho = rho_indep
# 	else:
# 		rho = eta * prev + np.sqrt(1. - eta ** 2) * rho_indep
...
# 	M = np.tril(M)
# 	dim = rho.shape[0]
# 	dist = npdist.MultivariateNormal(loc = np.zeros(dim), scale_tril = M)
# 	return dist.log_prob(rho)
